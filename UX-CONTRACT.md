...
该偏好只作用于已打开的对话；首页新建会话始终是 Command-Return 启动、Return 换行，
设置页与新建页的文案必须把这条写清楚，不能让两处说法互相矛盾。
...
视觉上三档不能排回同一列：目录名最重，任务缩进并位于浅分组中，会话再缩进一档且默认用次要色。
当前项用淡选中底加左侧短强调线标出。两个分区用分段标题和分隔线分开。
工作空间标题旁的加号创建新任务；目录、任务与任务标签加号保留各自预填上下文。
这些加号（含任务行「新建会话」与「会话数量 + 箭头」）始终可见可点，命中区不低于
`WandIconButtonStyle` 的 30 × 30 pt，可被 VoiceOver 读到；进入多选时隐藏任务加号，
不让多选过程中误开新建页。
折叠控件在筛选期间不可用时必须换成静态标签并明说原因（例如「筛选时保持展开」），
`.plain` 按钮被禁用后外观不变，不能留下「点了没反应」的假按钮。
...
返回已有草稿、打开归属浮层及收到任务列表变更时，重新读取可选任务与工作空间，
不能因草稿已初始化而冻结选项。刷新合并并发请求，保留输入、显式选择和当前任务绑定；
读取由草稿持有，关闭浮层不会取消共享请求或丢弃待刷新事件。失败保留上次列表，
在归属浮层提供重试，不重新套用服务器默认值。顶部、正文标题与任务按钮使用同一任务名解析，
列表刷新后同步显示新名称，不改变草稿 ID 或缓存键。

首条消息使用 IME-aware NSTextView，Return 换行、Command-Return 启动；输入法候选确认不触发创建。
新建会话页不读取聊天发送偏好，按钮提示必须写明 Command-Return。Command-L 聚焦首页输入。
默认值载入、绑定任务详情、创建请求中的状态分别可见；
...
| 拖看板卡片到「处理中」 | 卡片拖入列（该任务还没有任何会话） | 确认框列出将要使用的说明 | 对应看板列，并按确认结果决定是否派发 | 刷新后的任务状态与会话 | 派发失败后仍保留失败原因，不回滚状态 | 回到看板 | `TaskBoardView.swift` |
...
Git 检查器分别呈现加载、错误、非 Git 目录、状态缺失、干净与有改动；非 Git 目录不能显示
绿色“工作区干净”。所有文件状态（包含重命名和冲突）都计为改动；错误时不显示提交入口，
刷新中的旧状态不能启动提交。
...
右侧文件栏的根目录以会话 cwd 为准：会话已选定但 cwd 还没到时显示「正在读取工作目录」并等待，
不能用空路径去请求服务器默认目录（那会把用户带到别的项目）；任务或会话切换过程中不清空
壳层的当前选择，新快照到达后再替换。
...
会产生真实副作用的两条路径不能静默执行：拖卡片进「处理中」而该任务还没有任何会话时先确认
（确认框里列出将要发给 Agent 的说明），快捷提交在说明为空时先生成并回填说明与 Tag，
用户看到将要写进 git 的内容后再点一次才提交。两条都保留原有的失败提示与恢复入口。
普通复制、切换偏好等可逆动作不新增确认。
